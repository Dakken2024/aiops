...
@login_required
def high_risk_audit_list(request):
    """高危命令审计列表"""
    if not request.user.is_superuser:
        return redirect('dashboard')

    # 获取所有记录，按时间倒序
    logs = HighRiskAudit.objects.select_related('user', 'server').all().order_by('-created_at')

    return render(request, 'cmdb/high_risk_audit.html', {
        'logs': logs,
        'page_title': '高危操作审计'
    })
